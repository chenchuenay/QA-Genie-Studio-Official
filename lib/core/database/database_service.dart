...
class DatabaseService {
  ...
  static Future<void> _onUpgrade(
    ...
    // Safety: ensure column exists even if version is already 2 (e.g., partial migration)
    try {
      final columns = await db.rawQuery("PRAGMA table_info(test_cases)");
      final hasCaseJson = columns.any((col) => col['name'] == 'case_json');
      if (!hasCaseJson) {
        await db.execute('ALTER TABLE test_cases ADD COLUMN case_json TEXT');
      }
    } catch (e) {
      print('Failed to ensure case_json column: $e');
    }
  }
  ...
  static Future<void> replaceAllTestCases({
    required int suiteId,
    required List<FinalizedTestCase> cases,
  }) async {
    final db = await DatabaseService.db;
  ...
  static FinalizedTestCase _decodeTestCase(String jsonStr) {
    ...
    return FinalizedTestCase(
      ...
      steps: (map['steps'] as List).map((s) => TestStep.fromJson(s)).toList(),
