...
class UsageManager {
  ...
  static const _firstExportUsedKey = 'first_export_used';
  ...
  static Future<bool> canGenerate({bool afterRewardedAd = false}) async {
    if (SecurityBridge.instance.canBypassLimits) return true;
    final pro = await isPro();
    if (pro) return _checkServerQuota(type: 'generation', limit: _proDailyGenerationLimit);
    final freeCount = await getGenerationCount();
    final rewardedCount = await getRewardedGenerationCount();
    if (freeCount < _freeDailyGenerationLimit) return true;
    if (!afterRewardedAd) return false;
  ...
  static Future<void> incrementGeneration({bool rewarded = false}) async {
    ...
    await CloudAuthorityService.instance.trackUsage(type: 'generation', value: 1);
  }
  ...
  static Future<bool> isFirstExportFreeUsed() async {
    final prefs = await _prefs();
    return prefs.getBool(_firstExportUsedKey) ?? false;
  }

  static Future<void> markFirstExportUsed() async {
    final prefs = await _prefs();
    await prefs.setBool(_firstExportUsedKey, true);
  }
  ...
  static Future<void> incrementSummaryExport({bool rewarded = false}) async {
    ...
    await CloudAuthorityService.instance.trackUsage(type: 'summary_export', value: 1);
  }
  ...
  static Future<void> resetLimits() => resetDailyUsage();

  static Future<bool> _checkServerQuota({required String type, required int limit}) async {
    try {
      return await CloudAuthorityService.instance.validateQuota(type: type, limit: limit);
    } catch (_) {
      return false;
    }
  }
