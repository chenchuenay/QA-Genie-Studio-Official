...
class ExpectedResultGenerator {
  ...
  static String _positiveResult(ActionType action, EntityType entity, DomainContext domain, String platform, StateType targetState) {
    switch (action) {
      case ActionType.login:
        return 'Member is redirected to the dashboard/home page. The top navigation shows the member\'s display name and avatar. A welcome message confirms successful login. The session cookie/token is set and accessible for subsequent API calls.';
      case ActionType.logout:
        return 'Member is redirected to the login page. Session data is cleared from local storage. The back button does not expose authenticated content. All protected routes redirect to the login page.';
      case ActionType.authenticate:
        return 'Authentication succeeds. API returns HTTP 200 with a valid JWT access token and refresh token. Member profile data is returned in the response. The session is established and associated with the authenticated member.';
      case ActionType.refresh:
        return 'Token refresh succeeds. A new access token is issued with an extended expiration time. The old token is invalidated. The member remains authenticated without interruption to their current session.';
      case ActionType.reset:
        return 'Password reset completes successfully. A confirmation message displays: "Your password has been reset successfully." The member can immediately log in with the new password. The old password is invalidated. A confirmation email is sent.';
      case ActionType.create:
        if (entity == EntityType.record) {
          return 'A new record is created successfully. The record appears in the records list with a unique Record ID. All entered fields are saved accurately. A confirmation toast displays: "Record created successfully." The member is redirected to the record detail view.';
        }
      ...
      case ActionType.verify:
        return 'OTP/token verification succeeds. The member is granted access to the protected resource or action. The verification status is recorded. No session timeout occurs during the verification flow.';
      case ActionType.authorize:
        return 'OAuth authorization succeeds. The member is redirected to the application with an authorization code. The code is exchanged for access and refresh tokens. The requested scopes (email, profile) are granted.';
      case ActionType.add:
      ...
      case ActionType.cancel:
        return 'The $entity is cancelled successfully. The status changes to "Cancelled" with a visual indicator. If applicable, the slot or resource is released back to availability. A cancellation confirmation is sent to the member.';
  ...
  static String _negativeResult(ActionType action, EntityType entity, DomainContext domain, String platform, StateType targetState) {
    switch (action) {
      ...
      case ActionType.update:
        return 'Update fails. The system rejects the changes with an appropriate error message. The original data remains unchanged. No partial save occurs. The member can correct the input and retry.';
      case ActionType.delete:
      ...
      case ActionType.pay:
        return 'Payment is rejected. The payment gateway returns a decline code. The member sees a message: "Your card was declined. Please try a different payment method." No hold is placed. The order remains in unpaid state.';
      case ActionType.transfer:
      ...
      case ActionType.reschedule:
        return 'The reschedule request is rejected with an error message indicating the target time slot is unavailable. The original appointment remains unchanged with its original date and time. The member can select a different slot.';
      case ActionType.trigger:
      ...
      case ActionType.verify:
        return 'Verification fails. An error message displays: "Invalid or expired code. Please request a new code." Access to the protected resource is denied. The member can request a new OTP/token.';
      case ActionType.apply:
        return 'The coupon is rejected. An inline error message states the code is expired or invalid. The cart total remains unchanged. No discount is applied. The member can remove the code or try a different one.';
      default:
        return 'The operation is rejected with a clear error message. No system state changes occur. The member can correct the input and retry the operation. The error is logged for monitoring without exposing sensitive details.';
    }
  }
}
