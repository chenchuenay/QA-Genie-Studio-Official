...
class FinalizationPipeline {
  ...
  List<FinalizedTestCase> finalize(GenerationContext context, List<WorkingCase> cases) {
    return cases.map((wc) {
      return FinalizedTestCase(
        ...
        steps: List.unmodifiable(wc.steps.map((s) => FinalizedTestStep(
        ...
        metadata: wc.metadata.copy(), // Metadata is already mostly immutable-ish or copied
      );
    }).toList();
  }
}
