...
class DiversityEngine {
  ...
  DiversityResult evaluate(List<WorkingCase> cases) {
    ...
    final meetsTaxonomy =
        representedCount >= 5; // Enterprise bar: at least 5 distinct intents
  ...
  Set<String> _detectIntents(WorkingCase tc) {
    final text =
        '${tc.title} ${tc.expectedResult} ${tc.steps.map((s) => s.action).join(' ')}'
            .toLowerCase();
    ...
    if (text.contains('navigate') || text.contains('screen'))
      intents.add('navigation');
    if (text.contains('save') || text.contains('persist'))
      intents.add('persistence');
    if (text.contains('offline') || text.contains('retry'))
      intents.add('resilience');
    if (text.contains('timeout') || text.contains('race'))
      intents.add('concurrency');
