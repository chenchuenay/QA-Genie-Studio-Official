...
class SemanticValidator {
  ...
  SemanticValidationResult validate(GenerationContext context, List<WorkingCase> cases) {
  ...
  String _buildFingerprint(WorkingCase tc, String profile) {
    ...
    final failureMode = (tc.expectedResult.toLowerCase().contains('error') ||
  ...
  String _extractSemanticProfile(WorkingCase tc) {
    ...
    if (text.contains('payment') || text.contains('checkout')) return 'validation';
    if (text.contains('session') || text.contains('timeout')) return 'session';
    if (text.contains('accessibility') || text.contains('usability')) return 'usability';
