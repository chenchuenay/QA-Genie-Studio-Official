...
class PipelineAuditLogger {
  ...
  PipelineAuditReport buildReport() {
    return PipelineAuditReport(
      traceId: traceId,
      rejectedCases: List.unmodifiable(_rejectedCases),
      repairLog: List.unmodifiable(_repairLog),
      diversityBalance: UnmodifiableMapView(_diversityBalance),
      averageConfidence: _averageConfidence,
      fallbackTriggers: List.unmodifiable(_fallbackTriggers),
    );
  }
  ...
  Map<String, dynamic> toJson() {
    return {
      ...
      'averageConfidence': _averageConfidence,
      'rejectedCases': _rejectedCases
          .map((e) => {'title': e.title, 'reason': e.reason, 'stage': e.stage})
          .toList(),
