# QA Genie Studio — AI Agent Reference

**🔴 READ `MANIFESTO.md` BEFORE ANY ACTION. This app is the only hope. Every edit matters.**

---

## pending_agent_tasks.md

1. After discussing and I approve the tasks, I'll give a green signal (e.g. "proceed", "go on", "ok")
2. Only then you write the finalized approved task list into `pending_agent_tasks.md`
3. Mark each task as `[x]` completed as you finish them
4. After all tasks are done, clear the file entirely
...
adb connect 100.76.29.173:5555

- IPv6: `2001:8f8:1d0f:fdb9:d4c4:d7ff:fe57:ce71`
- Old: 10.184.222.252:5555
- Wi-Fi: 10.163.47.248:5555
...
QA Genie Studio is **not** a test management tool (TestRail, Zephyr, Qase, Xray) — it's a **generation layer upstream** of them. Target workflow:

1. Open QA Genie Studio → describe a feature → get structured test cases
2. Export to CSV / JSON / Xray format
3. Import into existing test management tool

Users keep their existing ecosystem. QA Genie Studio just removes the painful part — writing test cases from scratch.
