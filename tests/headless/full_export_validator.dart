...
void sanitizeSteps(List<TestCaseModel> cases) {
  for (final tc in cases) {
    for (final step in tc.steps) {
      if (step.action.trim().isEmpty) step.action = 'Perform the required interaction';
      if (step.expected.trim().isEmpty) step.expected = 'The system handles the interaction correctly';
    }
  }
}
...
Future<File?> exportAndCapture({required String type, required List<TestCaseModel> cases,
  required String module, required String feature, required String suiteDir}) async {
  final useCase = ExportTestCasesUseCase();
  try {
    await useCase.execute(type: type, cases: cases, moduleName: module, featureName: feature);
  } catch (e) {
    if (e is Exception && e.toString().contains('MissingPluginException')) {
      print('Share plugin not available in headless test, but file generation succeeded.');
    } else {
  ...
  final files = tempDir.listSync().whereType<File>().where((f) =>
    f.path.contains(module.replaceAll(' ', '_')) || f.path.contains(type)).toList();
...
Future<void> runValidator() async {
  ...
  if (!configFile.existsSync()) { print('Config missing'); return; }
  ...
  if (!trendFile.existsSync()) trendFile.writeAsStringSync('timestamp,platform,mode,avg_score\n');
  ...
  for (final entry in platforms.entries) {
    ...
    for (final count in [10, 20]) {
      ...
      final result = await service.execute(module: module, feature: feature, platform: platform, maxCases: count);
      ...
      await rawFile.writeAsString(const JsonEncoder.withIndent('  ').convert(cases.map((e) => e.toJson()).toList()));

      for (final fmt in ['excel', 'jira', 'xray', 'pdf']) {
        await exportAndCapture(type: fmt, cases: cases, module: module, feature: feature, suiteDir: suiteDir.path);
      }
  ...
  await File('test_results/quality_report.txt').writeAsString(reportBuf.toString());
  print('DONE');
}

void main() async { await runValidator(); }
