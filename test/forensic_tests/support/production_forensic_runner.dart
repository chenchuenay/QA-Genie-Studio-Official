...
import 'package:qa_genie/firebase/firebase_options.dart';
...
class ProductionForensicRunner {
  ...
  static Future<void> initialize() async {
    ...
    print('FORENSIC_FIREBASE_INIT_START');

    await Firebase.initializeApp(options: DefaultFirebaseOptions.android);

    print('FORENSIC_FIREBASE_INIT_SUCCESS');
    print('FIREBASE_APP=${Firebase.app().name}');

    _initialized = true;
  }

  // IMPORTANT:
  // NO AiGenerationStage.useTestCaller(...)
  //
  // This forces the pipeline to use:
  //
  // AiGenerationStage
  ...
  static Future<GenerationSession> execute({
    ...
    final baseDir = Directory('test_results/production_gen_results');
    ...
    pipelineLog.writeln('=== QA GENIE PRODUCTION FORENSIC PIPELINE LOG ===');
    ...
    pipelineLog.writeln('transport: CLOUD_FUNCTION');
    ...
    pipelineLog.writeln('endpoint: ${report.aiEndpoint ?? 'cloud_function'}');
    ...
    pipelineLog.writeln(
      jsonEncode(session.testCases.map(_toProductionJson).toList()),
    );
    ...
    final analyticalLine =
        ...
        '${report.structuralRejectedCount ?? 0},'
        '${report.semanticRejectedCount ?? 0},'
        '${report.realismRejectedCount ?? 0},'
        '${report.exportSafetyRejectedCount ?? 0},'
        '${report.repairedCount ?? 0},'
        '${report.fallbackCount ?? 0},'
        '${report.averageConfidence},'
        '${report.aiLatencyMs ?? 0}\n';
    ...
    final casesFile = File('test_results/last_production_generation.json');
  ...
  static Future<List<FinalizedTestCase>> loadLastGeneratedCases() async {
    final file = File('test_results/last_production_generation.json');

    if (!file.existsSync()) {
      throw Exception('No saved production generation found.');
    }
