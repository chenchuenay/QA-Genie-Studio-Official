...
- On the client side, an optional PII scrubber (emails, phone numbers, URLs, IPs, credit cards, API keys, JWTs, UUIDs) may redact sensitive patterns before the prompt leaves the device (controlled by environment configuration).
...
- **Fallback (development only)**: Google Gemini API (`generativelanguage.googleapis.com`) — called directly from the client with API key from local `.env`
...
Storage: Firestore (`usage/{uid}/metrics`, `deviceUsage/{deviceId}`, `analytics/global`)
...
| Title | User-entered text (max 40 chars) | Yes |
| Description | User-entered text (max 200 chars) | Yes |
| Steps to reproduce | User-entered text (optional) | No |
| Device model | Auto-detected (`device_info_plus`) | Optional (toggle ON by default) |
| App version | Auto-detected (`package_info_plus`) | Optional (toggle ON by default) |
| Platform | Auto-detected | Optional (toggle ON by default) |
...
| **Google Gemini API** | AI provider (dev fallback) | Test case generation | Full prompt text, API key |
...
- **At rest (local)**: SQLite database stored in the app's secure application support directory; no additional encryption layer
...
- **Guest tokens**: Mapped to device ID; retained until account deletion or after the deletion cooldown period expires (prevents quota abuse)
...
- **In-app**: Navigate to Account → Delete Account
- **Email**: Contact via the "Report an Issue" feature
...
QA Genie is not directed at children under 13 (or under 16 in the EU/UK). We do not knowingly collect personal information from children. If we learn that a child has provided personal data, we will delete it promptly. See our [Children's Privacy Policy](./CHILDRENS_PRIVACY.md) for details.

## 9. AI Disclaimer

Test cases generated by AI models are probabilistic in nature and may contain errors, inaccuracies, or hallucinations. All generated content should be reviewed by a qualified QA professional before use in any testing or production environment. See our [Terms of Service](https://gist.github.com/chenchuenay/74534f112ff634778fd98c48ae00bdcd) for full disclaimers.
...
For privacy questions, data deletion requests, or concerns, submit a report via the "Report an Issue" feature in the app's Support menu.
